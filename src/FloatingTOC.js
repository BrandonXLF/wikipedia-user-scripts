...
$(function(){
	...
	toc2.find('#toctogglecheckbox').attr('id','toctogglecheckbox2').prop('checked',data.expand ? false : true);
	!data.remove && $('#toc').find('#toctogglecheckbox').prop('checked',data.hide ? true : false);
	...
	toc2.find('h2').before('[',$('<a href="#">edit</a>').click(function(e){
		...
		var hide = new OO.ui.CheckboxInputWidget().setSelected(data.hide);
		var remove = new OO.ui.CheckboxInputWidget().setSelected(data.remove);
		var left = new OO.ui.CheckboxInputWidget().setSelected(data.left);
