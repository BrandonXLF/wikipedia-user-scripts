...
$(function(){
	...
	var area = $('<div style="position:absolute;top:0;bottom:0;left:0;right:0;background:rgba(0,0,0,0.3);">');
	var img = $('<img style="width:300px;pointer-events:none;user-select:none;" src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/7b/Dew_on_grass_Luc_Viatour.jpg/1920px-Dew_on_grass_Luc_Viatour.jpg">').on('load',function(){
	...
	var north = $('<div  style="position:absolute;left:0;right:0;top:0;height:2px;background:#000;cursor:n-resize;">');
	var south = $('<div style="position:absolute;left:0;right:0;bottom:0;height:2px;background:#000;cursor:s-resize">');
	var east = $('<div style="position:absolute;top:0;bottom:0;right:0;width:2px;background:#000;cursor:e-resize;">');
	var west = $('<div style="position:absolute;top:0;bottom:0;left:0;width:2px;background:#000;cursor:w-resize;">');
	var file = $('<input style="width:100%;margin:2px 0;padding:1px 2px;" placeholder="File" value="Dew on grass Luc Viatour.jpg">').on('change',function(){
			img.attr('src','https://en.wikipedia.org/wiki/Special:Filepath/' + file.val());
		});
	var size = $('<input type="number" style="width:100%;margin:2px 0;padding:1px 2px;" placeholder="Base width" value="300">').on('change',setcode);
	var textarea = $('<textarea readonly rows="10" style="width:100%;max-width:100%;margin:2px 0 10px;padding:1px 2px;resize:vertical;">');
	var width = $('<input type="number" style="width:100%;margin:2px 0;padding:1px 2px;">').on('change',update);
	var height = $('<input type="number" style="width:100%;margin:2px 0;padding:1px 2px;">').on('change',update);
	var left = $('<input type="number" style="width:100%;margin:2px 0;padding:1px 2px;">').on('change',update);
	var top = $('<input type="number" style="width:100%;margin:2px 0;padding:1px 2px;">').on('change',update);
	var location = $('<select style="width:100%;margin:2px 0;padding:1px 2px;"><option value="">(none)</option><option>none</option><option>right</option><option>left</option><option>center</option></select>').on('change',makecode);
	var desc = $('<input style="width:100%;margin:2px 0;padding:1px 2px;">').on('change',makecode);
				...
				.append($('<tr>')
					...
					.append($('<td style="width:100%;">')
						.append($('<div style="margin-top:8px;">')
							.append(textarea)	
							.append($('<button id="preview">Preview</button> <a href="https://en.wikipedia.org/wiki/Template:CSS_image_crop">Template:CSS image crop</a>').on('click',function(){
