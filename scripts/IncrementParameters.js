...
(function(unicode){
	$.when(mw.loader.using('oojs-ui'),$.ready).then(function(){
		$(mw.util.addPortletLink('p-tb', '#', 'Increment parameters')).click(function(e){
			e.preventDefault();
			var regex = {
				all: new RegExp('\\|(\\s*[' + unicode + ']?[' + unicode + '_-]*?\\s*)([1-9]\\d*)(\\s*[' + unicode + '_-]*?[' + unicode + '_-]?\\s*)=','g'),
				suffix: new RegExp('\\|(\\s*(?:header|label|data|rowclass|class)*?\\s*)([1-9]\\d*)(\\s*[' + unicode + '_-]*?[' + unicode + '_-]?\\s*)=','g'),
				infobox: new RegExp('\\|(\\s*(?:header|label|data|rowclass|class)*?\\s*)([1-9]\\d*)(\\s*)=','g')
			};
			var opts = new OO.ui.RadioSelectInputWidget( {
				value: 'all',
				options: [
					{
						data: 'all',
			...
			var increment = new OO.ui.NumberInputWidget({
				min: 0,
				step: 1,
				value: 1
			});
			var min = new OO.ui.NumberInputWidget({
				min: 0,
				step: 1,
				placeholder: '0'
			});
			var max = new OO.ui.NumberInputWidget({
				min: 0,
				step: 1,
				placeholder: 'Infinity'
			});
			var text = new OO.ui.MultilineTextInputWidget({
				rows: 10,
				maxRows: 999999,
				autosize: true,
				placeholder: 'Input'
				...
				if ( action ) {
					return new OO.ui.Process( function () {
						text.setValue(text.getValue().replace(regex[opts.getValue()], function (match, prefix, num, suffix) {
							if (parseInt(num, 10) < parseInt(min.getNumericValue() || 0, 10) || parseInt(num, 10) > parseInt(max.getNumericValue() || Infinity, 10)) {
								return match;
							}
