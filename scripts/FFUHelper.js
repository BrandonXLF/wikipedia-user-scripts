...
mw.hook('wikipage.content').add(function(){
	...
	function clickui (e) {
		...
		var section = parseInt(e.target.parentNode.getAttribute('data-section'));
		var secname = $(e.target).closest('h2').find('.mw-headline').attr('id');
		$('#ffuhelper').remove();
		...
		var user = '';
		var element = $(e.target).closest('h2').next();
		...
		$(this).closest('h2').after($('<p id="ffuhelper" style="border:1px solid ' + {comment:'grey',decline:'red',accept:'green'}[type] + ';border-radius:4px;padding:4px 0.5em;"></p>')
			...
			.append($('<div style="margin:4px 0 0 -4px;">')
				...
				.append($('<button style="margin:4px;">Save</button>').click(function(){
					...
					$.get(mw.config.get('wgScript'),{
						...
						var c = 0;
						var s = $('#ffunotify').prop('checked') && $('#ffuuser').val() ? 2 : 1;
						function updatePage() {
							c++;
							if (c === s) {
			...
			.append('<span style="display:block;margin-top:4px;">['
