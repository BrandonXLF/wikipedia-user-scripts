...
$.when(mw.loader.using('oojs-ui'), $.ready).then(function() {
	mw.hook('wikipage.diff').add(function() {
		...
		]})).selectItemByData('diff').$element
			.css('float', 'left')
			.css('margin', $('.ve-init-mw-diffPage-diffMode').length ? '0 0 0 8px' : '8px 0')
			.attr('id', 'comparerev-bar');
		...
		if ($('.ve-init-mw-diffPage-diffMode').length) {
			...
			$('#mw-content-text').prepend(parent);
		}
				...
				if (e.text() == 'Compare') {
					$('#comparerev-area').on('dblclick', function(e) {
						...
						if ($('#comparerev-old .mw-parser-output').get(0)) {
							$('body').append(
								'<div id="comparerev-x" style="top:0;bottom:0;width:1px;height:100%;background:blue;position:fixed;"></div>' +
								'<div id="comparerev-y" style="left:0;right:0;height:1px;width:100%;background:blue;position:fixed;"></div>'
							);
					...
					$('#comparerev-area').css('display', 'block').html(
						'<div style="width:50%;padding:3px 10px;box-sizing:border-box;float:left;" id="comparerev-old">' +
						'<div style="text-align:center;">Loading...</div>' +
						'</div>' +
						'<div style="width:50%;padding:3px 10px;box-sizing:border-box;float:left;" id="comparerev-new">' +
						'<div style="text-align:center;">Loading...</div>' +
						'</div>'
					);
