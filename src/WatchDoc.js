...
$(function(){
	if (mw.config.get('wgNamespaceNumber') == 10 || mw.config.get('wgNamespaceNumber') == 11) {
		$('#ca-watch a, #ca-unwatch a').click(function(){
			...
			var title = mw.config.get('wgPageName') + '/doc';
			...
			$.post(mw.config.get('wgScriptPath') + '/api.php', {
				...
				mw.notify($('<div>"<a href="' + mw.util.getUrl(a.watch[0].title) + '">' + a.watch[0].title + '</a>" and its ' + (mw.config.get('wgNamespaceNumber') == 10 ? 'talk page' : 'associated page') + ' have been ' + (unwatch ? 'removed' : 'added') + ' to your <a href="' + mw.util.getUrl('special:watchlist') + '">watchlist</a>.</div>'),{tag:'docpagewatch'});
			});
		});
	}
});