...
$.when(mw.loader.getScript('https://momentjs.com/downloads/moment.min.js'),$.ready).then(function(){
	...
	var DA_IMG = '<img style="height:1em;vertical-align:text-top;" title="Damaging" alt="Damaging" src="https://upload.wikimedia.org/wikipedia/commons/thumb/3/3d/Rating-Christgau-dud.svg/64px-Rating-Christgau-dud.svg.png">';
	var GF_IMG = '<img style="height:1em;vertical-align:text-top;" title="Good faith" alt="Good faith" src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/21/Ic_thumb_up_48px.svg/32px-Ic_thumb_up_48px.svg.png">';
    ...
    $.get(mw.config.get('wgScriptPath') + '/api.php', {
		...
		var id = '<a href="' + mw.util.getUrl('User:BrandonXLF/MoreDiffInfo#ID') + '">ID</a>: <a href="' + mw.config.get('wgScript') + '?oldid=' + info[0].revid + '">' + info[0].revid + '</a>';
		var size = ' | <a href="' + mw.util.getUrl('User:BrandonXLF/MoreDiffInfo#SIZE') + '">SIZE</a>: ' + info[0].size;
		var ores = '';
		...
		var timediff = ' | ' + moment(new Date(info[0].timestamp)).from(new Date(info[1].timestamp)).replace(/ ago$/,' later');
		id = '<a href="' + mw.util.getUrl('User:BrandonXLF/MoreDiffInfo#ID') + '">ID</a>: <a href="' + mw.config.get('wgScript') + '?oldid=' + info[1].revid + '">' + info[1].revid + '</a>';
		size = ' | <a href="' + mw.util.getUrl('User:BrandonXLF/MoreDiffInfo#SIZE') + '">SIZE</a>: ' + info[1].size + ' (<span style="color:' + (sizediff < 0 ? 'red' : sizediff > 0 ? 'green' : '') + '">' + (sizediff > 0 ? '+' : '') + sizediff + '</span>)';
		ores = '';
